report 50004 "Sales Invoice"
{
    ...
    begin
        ...
        IF Cust.FINDFIRST THEN
            CustomerOneLineAddress := Cust.Address;
        //KMT2016CU5 <<
    end;
        ...
        WITH SalesInvoiceLine DO BEGIN
            ...
            IF "Item Category Code" <> '' THEN BEGIN
                ...
                IF Vehicle.FINDFIRST THEN BEGIN
                    LineDescription[1] := 'Model Code' + FORMAT(CR) + FORMAT(LF);
                    LineDescription[1] += 'Model Version' + FORMAT(CR) + FORMAT(LF);
                    LineDescription[1] += 'Body Color' + FORMAT(CR) + FORMAT(LF);
                    LineDescription[1] += 'Frame No.' + FORMAT(CR) + FORMAT(LF);
                    LineDescription[1] += 'Motor No.' + FORMAT(CR) + FORMAT(LF);

                    //LineDescription[2] := COLON + "Product Group Code" + FORMAT(CR) + FORMAT(LF);
                    LineDescription[2] += COLON + "No." + FORMAT(CR) + FORMAT(LF);
                    LineDescription[2] += COLON + Vehicle."Body Color" + FORMAT(CR) + FORMAT(LF);
                    LineDescription[2] += COLON + Vehicle.VIN + FORMAT(CR) + FORMAT(LF);
                    LineDescription[2] += COLON + Vehicle."Engine No." + FORMAT(CR) + FORMAT(LF);
                END;
            END ELSE BEGIN
                LineDescription[1] := Description;
                LineDescription[2] := "Description 2";
            END;
            MESSAGE(LineDescription[1]);
            MESSAGE(LineDescription[2]);
        END;
    end;
