report 50442 "Landed Cost Sheet (Import)"
{
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem("Value Entry"; "Value Entry")
        {
            ...
            begin
                ...
                ItemCharge.RESET;
                ItemCharge.SETRANGE("No.", "Item Charge No.");
                ItemCharge.SETRANGE("Include in Purchase Amount", TRUE);
                IF ItemCharge.FINDFIRST THEN BEGIN
                    //IF ("Item Charge No." = '')  THEN BEGIN
                    ColumnName := Text1001;
                    IncludeInPurchAmt := "Cost Amount (Actual)";
                    //END;
                END ELSE BEGIN
                    IncludeInPurchAmt := 0;
                    IF ("Item Charge No." = '') THEN BEGIN
                ...
                /*IF ("Item Charge No." = '')  AND (ItemCharge."Include in Purchase Amount") THEN
                  ColumnName := Text1001
                ELSE
                  ColumnName := "Item Charge No.";*/
