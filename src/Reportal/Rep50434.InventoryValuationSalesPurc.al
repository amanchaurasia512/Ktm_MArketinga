report 50434 "Inventory Valuation Sales Purc"
{
    DefaultLayout = RDLC;
    RDLCLayout = './InventoryValuationSalesPurc.rdlc';
    Caption = 'Inventory Valuation Sales Purc';
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            column(PurchaseInvoicedValue; PurchaseInvoicedValue)
            {
            }
            column(PurchaseInvoicedQty; PurchaseInvoicedQty)
            {
            }
            column(PurchaseExpectedValue; PurchaseExpectedValue)
            {
            }
            column(PurchaseExpectedQty; PurchaseExpectedQty)
            {
            }
            column(SalesInvoicedValue; SalesInvoicedValue)
            {
            }
            column(SalesExpectedValue; SalesExpectedValue)
            {
            }
            column(SalesInvoicedQty; SalesInvoicedQty)
            {
            }
            column(SalesExpectedQty; SalesExpectedQty)
            {
            }
            column(EndingInvoicedValue; StartingInvoicedValue + IncreaseInvoicedValue - DecreaseInvoicedValue + PurchaseInvoicedValue - SalesInvoicedValue)
            {
            }
            column(EndingInvoicedQty; StartingInvoicedQty + IncreaseInvoicedQty - DecreaseInvoicedQty + PurchaseInvoicedQty - SalesInvoicedQty)
            {
            }
            column(EndingExpectedValue; StartingExpectedValue + IncreaseExpectedValue - DecreaseExpectedValue + PurchaseExpectedValue - SalesExpectedValue)
            {
                AutoFormatType = 1;
            }
            column(EndingExpectedQty; StartingExpectedQty + IncreaseExpectedQty - DecreaseExpectedQty + PurchaseExpectedQty - SalesExpectedQty)
            {
            }
            ...
            begin
                ...
                ExpCostPostedToGL := 0;
                //pram
                PurchaseExpectedQty := 0;
                PurchaseExpectedValue := 0;
                PurchaseInvoicedQty := 0;
                PurchaseInvoicedValue := 0;
                SalesExpectedQty := 0;
                SalesExpectedValue := 0;
                SalesInvoicedQty := 0;
                SalesInvoicedValue := 0;
                ...
                ValueEntry.CALCSUMS("Item Ledger Entry Quantity", "Cost Amount (Actual)", "Cost Amount (Expected)", "Invoiced Quantity");
                AssignAmounts(ValueEntry, PurchaseInvoicedValue, PurchaseInvoicedQty, PurchaseExpectedValue, PurchaseExpectedQty, 1);

                ValueEntry.SETRANGE("Posting Date", StartDate, EndDate);
                ValueEntry.SETFILTER(
                  "Item Ledger Entry Type", '%1|%2|%3',//|%4', //pram
                                                       //ValueEntry."Item Ledger Entry Type"::Purchase,
                ...
                ValueEntry.CALCSUMS("Item Ledger Entry Quantity", "Cost Amount (Actual)", "Cost Amount (Expected)", "Invoiced Quantity");
                AssignAmounts2(ValueEntry, SalesInvoicedValue, SalesInvoicedQty, SalesExpectedValue, SalesExpectedQty, -1);
                ...
                ValueEntry.CALCSUMS("Item Ledger Entry Quantity", "Cost Amount (Actual)", "Cost Amount (Expected)", "Invoiced Quantity");
                AssignAmounts2(ValueEntry, DecreaseInvoicedValue, DecreaseInvoicedQty, DecreaseExpectedValue, DecreaseExpectedQty, -1);
                ...
                IsEmptyLine := IsEmptyLine AND ((IncreaseInvoicedValue = 0) AND (IncreaseInvoicedQty = 0) AND (PurchaseInvoicedQty = 0)); //pram
                IsEmptyLine := IsEmptyLine AND ((DecreaseInvoicedValue = 0) AND (DecreaseInvoicedQty = 0) AND (SalesInvoicedQty = 0));
                IF ShowExpected THEN BEGIN
                    IsEmptyLine := IsEmptyLine AND ((IncreaseExpectedValue = 0) AND (IncreaseExpectedQty = 0) AND (PurchaseExpectedQty = 0));
                    IsEmptyLine := IsEmptyLine AND ((DecreaseExpectedValue = 0) AND (DecreaseExpectedQty = 0) AND (SalesExpectedQty = 0));
                END;
                ...
                PurchaseExpectedValue += PurchaseInvoicedValue;//pram
                SalesExpectedValue += SalesInvoicedValue;//pram
            ...
            begin
                ...
                //pram
                CurrReport.CREATETOTALS(PurchaseExpectedQty, PurchaseExpectedValue, PurchaseInvoicedQty, PurchaseInvoicedValue,
                        SalesExpectedQty, SalesExpectedValue, SalesInvoicedQty, SalesInvoicedValue);
            end;
        }
    }
    ...
    var
        ...
        IsEmptyLine: Boolean;
        PurchaseInvoicedValue: Decimal;
        PurchaseExpectedValue: Decimal;
        PurchaseInvoicedQty: Decimal;
        PurchaseExpectedQty: Decimal;
        SalesInvoicedValue: Decimal;
        SalesExpectedValue: Decimal;
        SalesInvoicedQty: Decimal;
        SalesExpectedQty: Decimal;
        SalesValue: Decimal;
    ...
    local procedure AssignAmounts2(ValueEntry: Record "Value Entry"; var InvoicedValue: Decimal; var InvoicedQty: Decimal; var ExpectedValue: Decimal; var ExpectedQty: Decimal; Sign: Decimal)
    begin
        InvoicedValue += ValueEntry."Sales Amount (Actual)" * Sign;
        InvoicedQty += ValueEntry."Invoiced Quantity" * Sign;
        ExpectedValue += ValueEntry."Cost Amount (Expected)" * Sign;
        ExpectedQty += ValueEntry."Item Ledger Entry Quantity" * Sign;
    end;
